...
while len(filtered_lines_oxy) > 1:
    ...
    if ones >= len(filtered_lines_oxy) / 2:
        filtered_lines_oxy = list(
            filter(lambda binary: binary[pos] == "1", filtered_lines_oxy)
        )
    else:
        filtered_lines_oxy = list(
            filter(lambda binary: binary[pos] == "0", filtered_lines_oxy)
        )
...
while len(filtered_lines_c02) > 1:
    ...
    if ones >= len(filtered_lines_c02) / 2:
        filtered_lines_c02 = list(
            filter(lambda binary: binary[pos] == "0", filtered_lines_c02)
        )
    else:
        filtered_lines_c02 = list(
            filter(lambda binary: binary[pos] == "1", filtered_lines_c02)
        )
